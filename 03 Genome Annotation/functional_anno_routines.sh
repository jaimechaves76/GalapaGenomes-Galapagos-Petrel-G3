...
# change to use /ccg/bin/OrthoDB12_downloads/Vertebrata_odb12 10Jan2025
...
function run_interproscan {
   ...
   log "$(interproscan.sh --version | head -n 1)"
...
function species_tally {
   annotated_fasta=$1
   [ ! -s "$annotated_fasta" ] && return

   make_species_tally.sh $annotated_fasta  # without 2nd arg writes to stdout
}
...
#  blastp_db /ccg/bin/orthodb_downloads/Vertebrata_OrthoDB_11 $tsv_dir/orthodb_verts.blastp.tsv
   blastp_db /ccg/bin/OrthoDB12_downloads/Vertebrata_odb12 $tsv_dir/orthodb_verts.blastp.tsv
}
...
#  diamond_blastp_more-sensitive /ccg/bin/orthodb_downloads/Vertebrata_OrthoDB_11 $tsv_dir/orthodb_verts.blastp.tsv
   diamond_blastp_more-sensitive /ccg/bin/OrthoDB12_downloads/Vertebrata_odb12 $tsv_dir/orthodb_verts.blastp.tsv
}
...
function touch_Standard_files {
   for s in Standard_gene_models.gff Standard_gene_models.faa Standard_gene_models.codingseq.fna Standard_gene_models.basic_gff_stats.txt; do
      [ -s "$s" ] && sleep 2 && touch $s
   done
}
...
function make_blasts_dir_dbs_info_file {

   function db_inf {
      stats_file=$1  # e.g. blast/nt_blastn.stats
      head -n 5 $stats_file | awk '{printf("%s ",$0)}END{printf("\n")}' | sed "s/\s*Longest sequence.*//"
   }

   [ -f blasts_dbs_info.txt ] && rm blasts_dbs_info.txt
   for stat in blasts/*.stats; do
      db_inf $stat >> blasts_dbs_info.txt
   done
}
...
function run_anno {
   ...
   make_blasts_dir_dbs_info_file  # info about when dbs made and number of sequences

   # 8 analyze blasts and make gene-models, gffs, faa and fna files
   make_gene_models
   touch_Standard_files  # Candidate is last one updated in make_gene_models but we want Standard as the newest

   # 9 create ipr distributions file, species tally file and combined basic stats file
   create_info_files
